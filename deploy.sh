...
if [ -f "ecommerce/backend/.env" ]; then
  ...
  ENV_VARS=$(grep -v '^#' ecommerce/backend/.env | grep -v '^$' | grep -v 'GOOGLE_CLOUD_PROJECT=' | grep -v 'GOOGLE_CLOUD_LOCATION=' | grep -v 'VERTEX_PROJECT_ID=' | grep -v 'VERTEX_LOCATION=' | grep -v 'MULTIMODAL_EMBEDDING_LOCATION=' | tr -d '\r' | paste -s -d "," -)
fi
...
gcloud run deploy cymbal-avatar \
  ...
  --min-instances 1 \
  --no-cpu-throttling \
  --set-env-vars "GOOGLE_CLOUD_PROJECT=gen-ai-4all,GOOGLE_CLOUD_LOCATION=us-central1,VERTEX_PROJECT_ID=gen-ai-4all,VERTEX_LOCATION=us-central1,MULTIMODAL_EMBEDDING_LOCATION=global${ENV_VARS:+,}${ENV_VARS}"
